...
def camera_loss_single(pred_pose_enc, gt_pose_enc, loss_type="l1"):
    ...
    """
    # Scale-align translation (first 3 dims) to GT before computing the loss.
    # Pretrained VGGT outputs metric-scale translations; our data loader
    # divides GT trans by per-batch scene_scale, so a naive comparison drags
    # the model away from its calibrated scale. Eval already scale-aligns
    # before computing trans_err (see eval/metrics.py:translation_error), so
    # we mirror that here. Detach `s` so gradient learns *shape*, not scale.
    # Clamp |s| to [0.1, 10]: without the cap, when pred_t collapses toward
    # zero (lite training failure mode), s explodes to ~1e6 to compensate and
    # the loss reads ≈0 even though the prediction is meaningless. Capping s
    # restores a useful gradient that pushes pred_t back toward sensible
    # magnitudes. Pretrained dense observed s≈1.7, well inside the cap.
    pred_t_raw = pred_pose_enc[..., :3]
    gt_t = gt_pose_enc[..., :3]
    if pred_t_raw.numel() > 0:
        pp = (pred_t_raw * pred_t_raw).sum().clamp(min=1e-8)
        s_t_raw = (gt_t * pred_t_raw).sum() / pp
        s_t = (s_t_raw.sign() * s_t_raw.abs().clamp(min=0.1, max=10.0)).detach()
        pred_t = pred_t_raw * s_t
    else:
        pred_t = pred_t_raw
    if loss_type == "l1":
        loss_T = (pred_t - gt_t).abs()
        loss_R = (pred_pose_enc[..., 3:7] - gt_pose_enc[..., 3:7]).abs()
        loss_FL = (pred_pose_enc[..., 7:] - gt_pose_enc[..., 7:]).abs()
    elif loss_type == "l2":
        loss_T = (pred_t - gt_t).norm(dim=-1, keepdim=True)
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    # Scale-align prediction to GT before computing the regression loss.
    # Pretrained VGGT outputs metric-scale depth/points; our data loader
    # divides GT by per-batch scene_scale, creating a scale mismatch that
    # would otherwise drag the model off its calibrated scale during
    # finetune. Eval already scale-aligns (see eval/metrics.py:depth_metrics
    # with scale_invariant=True), so we mirror that for training. `s` is
    # detached so gradient learns *shape*, not absolute scale.
    # Clamp s to [0.1, 10]: without the cap, lite training collapses pred to
    # ≈0 because s grows unbounded to compensate, leaving loss ≈0 with no
    # gradient to pull pred back. Capping forces a meaningful loss whenever
    # pred drifts an order of magnitude away from GT. Pretrained dense uses
    # s≈1.7 in practice, comfortably inside the cap.
    g_masked = gt[mask]
    p_masked = pred[mask]
    if mask.sum() > 100 and p_masked.numel() > 0:
        pp = (p_masked * p_masked).sum().clamp(min=1e-8)
        s_d_raw = (g_masked * p_masked).sum() / pp
        s_d = s_d_raw.clamp(min=0.1, max=10.0).detach()
        p_masked = p_masked * s_d
    loss_reg = torch.norm(g_masked - p_masked, dim=-1)
    ...
    # Clamp conf to [1e-3, ...] before log so that early-training low-conf predictions
    # cannot blow up the loss to ~20+ per pixel via -alpha * log(0).
    conf_masked = conf[mask].clamp(min=1e-3)
    loss_conf = gamma * loss_reg * conf_masked - alpha * torch.log(conf_masked)
